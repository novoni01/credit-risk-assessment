-- psql -d credit_risk -f database/validation.sql

-- Clean up kaggle accepted
...
--Clean up kaggle rejected staging table
...
WHERE 
    ...
    AND "Debt-To-Income Ratio" > 0;

--Clean up hdma accepted staging table
...
SELECT
    ...
    CASE
        ...
        WHEN applicant_credit_score_type IN ('7') THEN 'Multiple Models'
        WHEN applicant_credit_score_type IN ('8') THEN 'Other Model'
        WHEN applicant_credit_score_type IN ('9') THEN 'Not Applicable'
        ELSE 'Exempt'
    ...
    CASE
        ...
        WHEN co_applicant_credit_score_type IN ('7') THEN 'Multiple Models'
        WHEN co_applicant_credit_score_type IN ('8') THEN 'Other Model'
        WHEN co_applicant_credit_score_type IN ('9') THEN 'Not Applicable'
        WHEN co_applicant_credit_score_type IN ('10') THEN 'No co-applicant'
        ELSE 'Exempt'
    ...
    loan_to_value_ratio,
    CASE
        WHEN derived_loan_product_type LIKE 'Conventional%' THEN 'Conventional'
        WHEN derived_loan_product_type LIKE 'FHA%' THEN 'FHA'
        WHEN derived_loan_product_type LIKE 'VA%' THEN 'VA'
        WHEN derived_loan_product_type LIKE 'FSA/RHS%' THEN 'FSA/RHS'
    END AS loan_program,

    CASE
        WHEN derived_loan_product_type LIKE '%First Lien' THEN 'First Lien'
        WHEN derived_loan_product_type LIKE '%Subordinate Lien' THEN 'Subordinate Lien'
    END AS lien_status,
...
WHERE 
    ...
    AND NULLIF(debt_to_income_ratio, '')::NUMERIC BETWEEN 0 AND 35
...
--Clean up rejected hdma staging table
...
WHERE 
    ...
    AND income BETWEEN 0 AND 3000000
    AND debt_to_income_ratio <> ''
    AND NULLIF(debt_to_income_ratio, '')::NUMERIC > 0
